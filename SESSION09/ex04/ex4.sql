...
BEGIN
	SET total_balance = (SELECT SUM(Balance) FROM Accounts WHERE CustomerID = _CustomerID);
END //
DELIMITER ;

SET @total_balance = 0;
CALL GetTotalBalance(3,@total_balance);
...
DELIMITER //
CREATE procedure 
IncreaseEmployeeSalary(IN 
_EmployeeID int,
INOUT current_salary DECIMAL(10,2)) 
BEGIN
	SET current_salary = current_salary * 1.1;
    UPDATE Employees SET Salary = current_salary WHERE EmployeeID = _EmployeeID;
END //
DELIMITER ;
SELECT * FROM Employees;

SELECT Salary INTO @sessions From Employees WHERE EmployeeID = 4;
CALL IncreaseEmployeeSalary(4,@sessions);
SELECT @sessions AS 'Muc luong sau khi thay doi';
...
-- ex04
-- TransferMoney: Chuyển tiền thành công khi tài khoản gửi có đủ số dư, số tiền được cộng trừ chính xác; nếu không đủ số dư thì không giao dịch và số tiền chuyển = 0.
DELIMITER //
CREATE procedure 
TransferMoney(IN 
_sendID int,
OUT reciverID int, amount DECIMAL(10,2)) 
BEGIN
	SET current_salary = current_salary * 1.1;
    UPDATE Employees SET Salary = current_salary WHERE EmployeeID = _EmployeeID;
END //
...
-- UpdateSalaryByID: Lương nhân viên được cập nhật đúng theo quy tắc (tăng 10% nếu <20 triệu, tăng 5% nếu ≥20 triệu).
...
SET @sessions = (SELECT Salary From Employees WHERE EmployeeID = 1);
CALL IncreaseEmployeeSalary(1,@sessions);
SELECT @sessions AS 'Muc luong sau khi thay doi';
